...
<div class="grid grid-cols-1 gap-7">
    <div class="flex justify-end">
        <div x-data="modals">
            <div class="flex items-center ">
                <button type="button" title="Create User" class="btn text-black dark:text-white border hover:bg-indigo-300 dark:hover:bg-indigo-300 border-indigo-300 bg-transparent hover:text-black" @click="toggle">
                    Create User
                </button>
            </div>
                    ...
                    <div x-show="open" x-transition x-transition.duration.300 class="bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8">
                        ...
                        <div class="p-5">
                            <div class="text-sm text-black dark:text-white flex flex-col gap-3">
                                <form action="<?= base_url("api/admin/all-master/users/create") ?>" method="post" id="doc-upload" data-reload="true" class="grid grid-cols-2 gap-3  fetch-form" enctype="multipart/form-data">

                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">User name</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type branch name unique " name="username" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Email</label>
                                        <input type="email" value="" required minlength="2" placeholder="...Type Email  unique " name="email" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">First name</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type First name" name="firstname" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Last Name</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type Last Name" name="lastname" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Telephone</label>
                                        <input type="number" value="" required minlength="2" placeholder="...Type telephone" name="telephone" class="form-input">
                                    </div>
    ...
    <?php if (count($Dataset) == 0) : ?>
        <div class="border border-black/10 dark:border-white/10 p-5 flex justify-center items-center rounded-md">
            <div>
                <p class="text-sm font-semibold">No Users</p>
            </div>
        </div>
    <?php else : ?>
                    ...
                    <tbody>
                        <?php foreach ($Dataset as $index => $item) : ?>
                            <tr>
                                ...
                                <td>
                                    <?= $Options['branches'][$item['branch']] ?? 'Global Branch' ?>
                                </td>

                                <?php if ((int)$item['in_active']) : ?>
                                    <td class="flex items-center text-black/40 dark:text-white/40 py-3">
                                ...
                                <?php else : ?>
                                    <td class="flex items-center text-xs text-xs text-[#4AA785]  py-3">
                                        <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M11 8C11 9.65685 9.65685 11 8 11C6.34315 11 5 9.65685 5 8C5 6.34315 6.34315 5 8 5C9.65685 5 11 6.34315 11 8Z" fill="currentcolor"></path>
                                        </svg>
                                        <p>Active</p>
    ...
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8">
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/users/update") ?>" method="post" data-reload="true" class="grid grid-cols-2 gap-3 fetch-form" enctype="multipart/form-data">
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">User name</label>
                        <input type="text" value="" required minlength="2" id="edit-form-username" placeholder="...Type branch name unique " name="username" class="form-input">
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Email</label>
                        <input type="email" value="" required minlength="2" id="edit-form-email" placeholder="...Type Email  unique " name="email" class="form-input">
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">First name</label>
                        <input type="text" value="" required minlength="2" id="edit-form-firstname" placeholder="...Type First name" name="firstname" class="form-input">
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Last Name</label>
                        <input type="text" value="" required minlength="2" id="edit-form-lastname" placeholder="...Type Last Name" name="lastname" class="form-input">
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Telephone</label>
                        <input type="number" value="" required minlength="2" id="edit-form-telephone" placeholder="...Type telephone" name="telephone" class="form-input">
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Visiblity</label>
                        <?= form_dropdown('visiblity', $Options['visiblity'], '', 'id="edit-form-visiblity" required class="form-select py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;"');
                        ?>
                    </div>
                    ...
                    <div class="flex col-span-2 justify-end items-center mt-8 gap-4">
...
<div class="modal fade fixed inset-0 bg-black/60 dark:bg-white/10 z-[999] hidden overflow-y-auto" id="delete-modal" tabindex="-1" aria-labelledby="delete-modal" data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden="true">
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/users/delete") ?>" method="post" data-reload="true" class="flex  flex-col fetch-form" enctype="multipart/form-data">
            ...
            __init: () => {
                ...
                myModalEl.addEventListener('show.bs.modal', event => {
                    try {
                        ...
                        const firstname = parent.data('firstname');
                        const lastname = parent.data('lastname');

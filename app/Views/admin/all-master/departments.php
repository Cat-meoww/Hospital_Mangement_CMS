...
<div class="grid grid-cols-1 gap-7">
    <div class="flex justify-end">
        <div x-data="modals">
            <div class="flex items-center ">
                <button type="button" title="Create department" class="btn text-black dark:text-white border hover:bg-indigo-300 dark:hover:bg-indigo-300 border-indigo-300 bg-transparent hover:text-black" @click="toggle">
                    Create Department
                </button>
            </div>
                ...
                <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
                    <div x-show="open" x-transition x-transition.duration.300 class="bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                        <div class="flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                            <h5 class="font-semibold text-lg">Create department</h5>
                        ...
                        <div class="p-5">
                            <div class="text-sm text-black dark:text-white flex flex-col gap-3">
                                <form action="<?= base_url("api/admin/all-master/departments/create") ?>" method="post" id="doc-upload" data-reload="true" class="flex gap-3 flex-col fetch-form" enctype="multipart/form-data">

                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Department name</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type department name unique " name="name" class="form-input">
                                    </div>
                                    <div class="relative">
    ...
    <?php if (count($Dataset) == 0) : ?>
        <div class="border border-black/10 dark:border-white/10 p-5 flex justify-center items-center rounded-md">
            <div>
                <p class="text-sm font-semibold">No Departments</p>
            </div>
        </div>
    <?php else : ?>
                    ...
                    <tbody>
                        <?php foreach ($Dataset as $index => $item) : ?>
                            <tr>
                                ...
                                <?php endif; ?>
                                <td>
                                    <div class="flex gap-1 flex-row justify-center" data-id="<?= $item->id ?>" data-name="<?= esc($item->name) ?>" data-description="<?= esc($item->description) ?>" data-slug="<?= esc($item->slug) ?>" data-visiblity="<?= $item->visibility ?>">
    ...
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            <div class="modal-header flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                <h5 class="font-semibold text-lg">Edit Departments</h5>
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/departments/update") ?>" method="post" data-reload="true" class="flex gap-3 flex-col fetch-form" enctype="multipart/form-data">
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Department name</label>
                        <input type="text" id="form-department-name" value="" required minlength="2" placeholder="...Type department name unique " name="name" class="form-input">
                    </div>
                    <div class="relative">
                        <textarea id="form-department-description" name="description" rows="2" class="block rounded-lg px-5 pb-4 pt-[38px] w-full text-black dark:text-white bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 appearance-none focus:outline-none focus:ring-0 focus:border-black/10 dark:focus:border-white/10 peer" placeholder=" "></textarea>
                        <label for="form-department-description" class="absolute text-sm text-black/40 dark:text-white/40 duration-300 transform -translate-y-2 scale-90 top-6 z-10 origin-[0] left-5 peer-focus:text-black/40 dark:peer-focus:text-white/40 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-90 peer-focus:-translate-y-2">
                            Description
                        </label>
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Enter Slug</label>
                        <input type="text" id="form-department-slug" value="" required minlength="2" placeholder="...Type slug  unique " pattern="[A-Za-z0-9\-]+" name="slug" class="form-input">
                    </div>



                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <?= form_dropdown('visiblity', $Options['visiblity'], '', 'id="form-department-visiblity" required class="form-select py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;"'); ?>
                    </div>
                    <input type="hidden" name="id" id="form-department-id">
                    <div class="msg-block">
...
<div class="modal fade fixed inset-0 bg-black/60 dark:bg-white/10 z-[999] hidden overflow-y-auto" id="delete-modal" tabindex="-1" aria-labelledby="delete-modal" data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden="true">
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/departments/delete") ?>" method="post" data-reload="true" class="flex  flex-col fetch-form" enctype="multipart/form-data">
        ...
        const editModal = {
            ...
            __init: () => {
                ...
                myModalEl.addEventListener('show.bs.modal', event => {
                    ...
                    $("#form-department-id").val(id);

                    $("#form-department-name").val(name);
                    $("#form-department-description").text(description);
                    $("#form-department-slug").val(slug);
                    $("#form-department-visiblity").val(visiblity);
                })
            }
        }
        editModal.__init();

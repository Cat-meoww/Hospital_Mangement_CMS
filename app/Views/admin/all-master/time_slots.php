...
<div class="grid grid-cols-1 gap-7">
                ...
                <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
                    <div x-show="open" x-transition x-transition.duration.300 class="bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                        <div class="flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                            <h5 class="font-semibold text-lg">Create Time Slot</h5>
                        ...
                        <div class="p-5">
                            <div class="text-sm text-black dark:text-white flex flex-col gap-3">
                                <form action="<?= base_url("api/admin/all-master/time-slots/create") ?>" method="post" id="doc-upload" data-reload="true" class="flex flex-col gap-3  fetch-form" enctype="multipart/form-data">

                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Start Time</label>
                                        <input type="time" value="" required minlength="2" placeholder="... Set Start Time " name="start_time" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">End Time</label>
                                        <input type="time" value="" required minlength="2" placeholder="...Set End Time " name="end_time" class="form-input">
                                    </div>
    ...
    <?php if (count($Dataset) == 0) : ?>
        <div class="border border-black/10 dark:border-white/10 p-5 flex justify-center items-center rounded-md">
            <div>
                <p class="text-sm font-semibold">No Time Slot</p>
            </div>
        </div>
    <?php else : ?>
        <div x-data="basic" class="border bg-lightwhite dark:bg-white/5 dark:border-white/10 border-black/10 p-5 rounded-md">
            <div class="px-2 py-1 mb-4">
                <h2 class="text-lg font-semibold"> Time Slot </h2>
            </div>
            <div class>
                <table id="dt-table" class="whitespace-nowrap table-hover table-bordered">
                    ...
                    <tbody>
                        <?php foreach ($Dataset as $index => $item) : ?>
                            <tr>
                                <td><?= $index + 1 ?></td>
                                <td><?= analogueFormate($item->start_time) ?></td>
                                <td><?= analogueFormate($item->end_time) ?></td>

                                <td>
                                    <div class="flex gap-1 flex-row justify-center" <?= renderDataAttributes($item, ['id', 'start_time', 'end_time']) ?>>
    ...
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/time-slots/delete") ?>" method="post" data-reload="true" class="flex  flex-col fetch-form" enctype="multipart/form-data">

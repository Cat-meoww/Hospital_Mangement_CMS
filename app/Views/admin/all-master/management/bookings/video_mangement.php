                ...
                <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
                    <div x-show="open" x-transition x-transition.duration.300 class="bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                        <div class="flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                            <h5 class="font-semibold text-lg">Add Doctor</h5>
                            ...
                            <div class="text-sm text-black dark:text-white flex flex-col gap-3">
                                <form action="<?= base_url("api/admin/all-master/bookings/video/add-doctor") ?>" method="post" id="doc-upload" data-reload="true" class="flex flex-col gap-3  fetch-form" enctype="multipart/form-data">
                                    ...
                                    <div class="flex justify-end items-center mt-4 gap-4 col-span-2">
                                        <button type="submit" class="btn text-black dark:text-white border hover:bg-lightgreen-100 dark:hover:bg-lightgreen-100 border-lightgreen-100 bg-transparent hover:text-black">Add</button>
                                    </div>
    ...
    <?php else : ?>
        <div x-data="basic" class="border bg-lightwhite dark:bg-white/5 dark:border-white/10 border-black/10 p-5 rounded-md">
            <div class="px-2 py-1 mb-4">
                <h2 class="text-lg font-semibold">Video Consultation Doctors </h2>
            </div>
            <div class>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="flex gap-1 flex-row justify-center" data-id="<?= $item->id ?>" data-slots="<?= esc($item->slots) ?>">
                                        ...
                                        <button type="button" title="Edit Slot" data-bs-toggle="modal" data-bs-target="#edit-modal" class="p-1 text-black dark:text-white">
        ...
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8">
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/bookings/video/update-doctor-slot") ?>" method="post" data-reload="true" class="flex gap-3 flex-col   fetch-form" enctype="multipart/form-data">
                    ...
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Slots</label>
                        <?= form_dropdown('timeslots[]', $Options['time_slots'], [], 'id="form-branch-doctors" multiple required width="100%" data-placeholder="Select an option" class="select2 form-select !bg-none py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;"'); ?>
                    </div>
...
<div class="modal fade fixed inset-0 bg-black/60 dark:bg-white/10 z-[999] hidden overflow-y-auto" id="delete-modal" tabindex="-1" aria-labelledby="delete-modal" data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden="true">
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/bookings/video/delete-doctor") ?>" method="post" data-reload="true" class="flex  flex-col fetch-form" enctype="multipart/form-data">
            ...
            __init: () => {
                ...
                myModalEl.addEventListener('show.bs.modal', async (event) => {
                    ...
                    try {
                        ...
                        $("#form-branch-doctors option").prop("selected", false);


                        $.each(slots.split(","), function(i, e) {

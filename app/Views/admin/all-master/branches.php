    ...
    <div class="flex justify-end">
        <div x-data="modals">
            <div class="flex items-center ">
                <button type="button" title="Create branch" class="btn text-black dark:text-white border hover:bg-indigo-300 dark:hover:bg-indigo-300 border-indigo-300 bg-transparent hover:text-black" @click="toggle">
                    Create Branch
                </button>
            </div>
                ...
                <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
                    <div x-show="open" x-transition x-transition.duration.300 class="bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8">
                        <div class="flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                            <h5 class="font-semibold text-lg">Create Branch</h5>
                        ...
                        <div class="p-5">
                            <div class="text-sm text-black dark:text-white flex flex-col gap-3">
                                <form action="<?= base_url("api/admin/all-master/branches/create") ?>" method="post" id="doc-upload" data-reload="true" class="grid grid-cols-2 gap-3  fetch-form" enctype="multipart/form-data">
                                    ...
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Address</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type address   " name="address" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <?= form_dropdown('location', $Options['locations'], '', 'required class="form-select py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;"'); ?>
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Email</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type Email " name="email" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Gmap url</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type Google map url " name="gmap" class="form-input">
                                    </div>
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Telephone Number</label>
                                        <input type="text" value="" required minlength="2" placeholder="...Type branch phone number" name="telephone" class="form-input">
                                    </div>
                    ...
                    <tbody>
                        <?php foreach ($Dataset as $index => $item) : ?>
                            <tr>
                                ...
                                <?php endif; ?>
                                <td>
                                    <div class="flex gap-1 flex-row justify-center" data-id="<?= $item->id ?>" data-name="<?= esc($item->name) ?>" data-description="<?= esc($item->description) ?>" data-slug="<?= esc($item->slug) ?>" data-location="<?= esc($item->location_id) ?>" data-email="<?= esc($item->email) ?>" data-address="<?= esc($item->address) ?>"  data-gmap="<?= esc($item->gmap) ?>" data-telephone="<?= esc($item->telephone) ?>"  data-visiblity="<?= $item->visibility ?>">
    ...
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-5xl my-8">
            <div class="modal-header flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                <h5 class="font-semibold text-lg">Edit Branch</h5>
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/branches/update") ?>" method="post" data-reload="true" class="grid grid-cols-2 gap-3  fetch-form" enctype="multipart/form-data">
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Branch name</label>
                        <input type="text" id="form-branch-name" value="" required minlength="2" placeholder="...Type branch name unique " name="name" class="form-input">
                    </div>
                    ...
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Enter Slug</label>
                        <input type="text"  id="form-branch-slug" required minlength="2" placeholder="...Type slug  unique " pattern="[A-Za-z0-9\-]+" name="slug" class="form-input">
                    </div>

                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Address</label>
                        <input type="text"  id="form-branch-address" required minlength="2" placeholder="...Type address   " name="address" class="form-input">
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <?= form_dropdown('location', $Options['locations'], '', 'id="form-branch-location" required class="form-select py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;"'); ?>
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Email</label>
                        <input type="text"  id="form-branch-email" required minlength="2" placeholder="...Type Email " name="email" class="form-input">
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Gmap url</label>
                        <input type="text"  id="form-branch-gmap" required minlength="2" placeholder="...Type Google map url " name="gmap" class="form-input">
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Telephone Number</label>
                        <input type="text" id="form-branch-telephone" required minlength="2" placeholder="...Type branch phone number" name="telephone" class="form-input">
                    </div>


                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <?= form_dropdown('visiblity', $Options['visiblity'], '', 'id="form-branch-visiblity" required class="form-select py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;"'); ?>
                    </div>
                    <div class="relative col-span-2">
                        <textarea id="form-branch-description" name="description" rows="2" class="block rounded-lg px-5 pb-4 pt-[38px] w-full text-black dark:text-white bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 appearance-none focus:outline-none focus:ring-0 focus:border-black/10 dark:focus:border-white/10 peer" placeholder=" "></textarea>
                        <label for="form-branch-description" class="absolute text-sm text-black/40 dark:text-white/40 duration-300 transform -translate-y-2 scale-90 top-6 z-10 origin-[0] left-5 peer-focus:text-black/40 dark:peer-focus:text-white/40 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-90 peer-focus:-translate-y-2">
                            Description
                        </label>
                    </div>
...
<div class="modal fade fixed inset-0 bg-black/60 dark:bg-white/10 z-[999] hidden overflow-y-auto" id="delete-modal" tabindex="-1" aria-labelledby="delete-modal" data-bs-backdrop="static" data-bs-keyboard="false" aria-hidden="true">
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/branches/delete") ?>" method="post" data-reload="true" class="flex  flex-col fetch-form" enctype="multipart/form-data">
        ...
        const editModal = {
            ...
            __init: () => {
                ...
                myModalEl.addEventListener('show.bs.modal', event => {
                    ...
                    const location = parent.data('location');
                    const email = parent.data('email');
                    const address = parent.data('address');
                    const telephone = parent.data('telephone');
                    const gmap = parent.data('gmap');
                    ...
                    $("#form-branch-description").text(description);
                    $("#form-branch-slug").val(slug);
                    $("#form-branch-location").val(location);
                    $("#form-branch-email").val(email);
                    $("#form-branch-address").val(address);
                    $("#form-branch-gmap").val(gmap);
                    $("#form-branch-telephone").val(telephone);
                    $("#form-branch-visiblity").val(visiblity);
                })
            }
        }
        editModal.__init();

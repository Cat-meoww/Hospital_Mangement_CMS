...
<form action="<?= base_url("api/admin/cms/blog/update") ?>" method="post" data-reload="true" autocomplete="off" class="grid grid-cols-1 gap-3 fetch-form">
    <input type="hidden" name="id" value="<?= $blog->id ?>">
    <div class="flex flex-wrap items-start justify-between gap-3 mb-4">
        <h2 class="text-lg font-semibold">Update Blog </h2>
        <div class="flex items-center gap-2">
            <a href="<?= base_url('admin/cms-page/blog') ?>" class="btn px-2 py-[5px] text-xs">Back</a>
        ...
        <div class="bg-lightwhite dark:bg-white/5 rounded-2xl p-6">
            ...
            <div class=" grid grid-flow-row gap-4">
                <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10 dark:border-white/10">
                    <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Slug</label>
                    <input required type="text" value="<?= $blog->slug ?>" name="slug" placeholder="... Type" class="form-input">
                </div>
                <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10 dark:border-white/10">
                    <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Meta Title</label>
                    <input required type="text" value="<?= $blog->meta_title ?>" name="meta_title" placeholder="... Type" class="form-input">
                </div>
                <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10 dark:border-white/10">
                    <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Meta Description</label>
                    <input required type="text" value="<?= $blog->meta_description ?>" name="meta_description" placeholder="... Type" class="form-input">
                </div>
            </div>
        </div>
        <div class="bg-lightwhite dark:bg-white/5 rounded-2xl p-6">
            <div class="flex flex-wrap items-start justify-between gap-3 mb-4">
                <h2 class="text-sm font-semibold">Blog Data</h2>
            </div>
            <div class=" grid grid-flow-row gap-4 text-black">
                <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10 dark:border-white/10">
                    ...
                    <input required type="text" value="<?= $blog->heading ?>" name="cms_heading" placeholder="... Type" class="form-input">
                </div>
                <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10 dark:border-white/10">
                    <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Display Image (Recommended 16:9)</label>
                    <input type="file" accept="image/*" require name="hero_image" class="form-input">
                    <div class=" flex justify-center">
                        <img width="500" loading="lazy" src="<?= base_url("/uploads/blogs/$blog->hero_image")   ?>" alt="">
                    </div>
                </div>

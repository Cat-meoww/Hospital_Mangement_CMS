    ...
    <div class="flex justify-end">
        <div x-data="modals">
            <div class="flex items-center ">
                <button type="button" title="Create Faq" class="btn text-black dark:text-white border hover:bg-indigo-300 dark:hover:bg-indigo-300 border-indigo-300 bg-transparent hover:text-black" @click="toggle">
                    Create FAQ
                </button>
            </div>
                ...
                <div class="flex items-center justify-center min-h-screen px-4" @click.self="open = false">
                    <div x-show="open" x-transition x-transition.duration.300 class="bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
                        <div class="flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                            <h5 class="font-semibold text-lg">Create FAQ</h5>
                        ...
                        <div class="p-5">
                            <div class="text-sm text-black dark:text-white flex flex-col gap-3">
                                <form action="<?= base_url("api/admin/all-master/cms/faq/create") ?>" method="post" id="doc-upload" data-reload="true" class="flex gap-3 flex-col fetch-form" enctype="multipart/form-data">
                                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Question</label>
                                        <input type="text" value="" required minlength="2" placeholder="Your question  ..." name="question" class="form-input">
                                    </div>
                                    <div class="relative">
                                        <textarea id="floating_filled" name="answer" rows="5" class="block rounded-lg px-5 pb-4 pt-[38px] w-full text-black dark:text-white bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 appearance-none focus:outline-none focus:ring-0 focus:border-black/10 dark:focus:border-white/10 peer" placeholder=" "></textarea>
                    ...
                    <tbody>
                        <?php foreach ($Faqs as $index => $Faq) : ?>
                            <tr>
                                ...
                                <?php endif; ?>
                                <td>
                                    <div class="flex gap-1 flex-row justify-center" data-id="<?= $Faq->id ?>" data-question="<?= esc($Faq->question) ?>" data-answer="<?= esc($Faq->answer) ?>" data-visiblity="<?= $Faq->visibility ?>">
    ...
    <div class="modal-dialog flex items-center justify-center min-h-screen px-4">
        <div class="modal-content bg-white dark:bg-black relative shadow-3xl border-0 p-0 rounded-lg overflow-hidden w-full max-w-lg my-8">
            <div class="modal-header flex bg-white dark:bg-black border-b border-black/10 dark:border-white/10 items-center justify-between px-5 py-3">
                <h5 class="font-semibold text-lg">Edit Faq</h5>
            ...
            <div class="modal-body p-5">
                <form action="<?= base_url("api/admin/all-master/cms/faq/update") ?>" method="post" data-reload="true" class="flex gap-3 flex-col fetch-form" enctype="multipart/form-data">
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <label class="block text-xs text-black/40 dark:text-white/40 mb-1">Question</label>
                        <input type="text" value="" id="form-Faq-question" required minlength="2" placeholder="Question ..." name="question" class="form-input">
                    </div>
                    <div class="relative">
                        <textarea id="form-Faq-answer" name="answer" rows="5" class="block rounded-lg px-5 pb-4 pt-[38px] w-full text-black dark:text-white bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 appearance-none focus:outline-none focus:ring-0 focus:border-black/10 dark:focus:border-white/10 peer" placeholder=" "></textarea>
                        <label for="form-Faq-answer" class="absolute text-sm text-black/40 dark:text-white/40 duration-300 transform -translate-y-2 scale-90 top-6 z-10 origin-[0] left-5 peer-focus:text-black/40 dark:peer-focus:text-white/40 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-90 peer-focus:-translate-y-2">
                            Answer
                        </label>
                    </div>
                    <div class="relative bg-white dark:bg-white/5 py-4 px-5 rounded-lg border border-black/10">
                        <select name="visiblity" id="form-Faq-visiblity" class="form-select py-2.5 px-4 w-full text-black dark:text-white border border-black/10 dark:border-white/10 rounded-lg placeholder:text-black/20 dark:placeholder:text-white/20 focus:border-black dark:focus:border-white/10 focus:ring-0 focus:shadow-none;">
                            <option value="Public">Public</option>
                            <option value="Private">Private</option>
                        </select>
                    </div>
                    <input type="hidden" name="id" id="form-Faq-id">
                    <div class="msg-block">
        ...
        const editModal = {
            ...
            __init: () => {
                ...
                myModalEl.addEventListener('show.bs.modal', event => {
                    ...
                    const question = parent.data('question');
                    const answer = parent.data('answer');
                    ...
                    $("#form-Faq-id").val(id);
                    $("#form-Faq-question").val(question);
                    $("#form-Faq-answer").text(answer);
                    $("#form-Faq-visiblity").val(visiblity);
                })
            }
        }
        editModal.__init();
